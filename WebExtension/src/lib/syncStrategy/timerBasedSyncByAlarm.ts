...
export class TimerBasedSyncByAlarm implements ISyncStrategy {
  ...
  async onUrlChange(
    ...
    let handling = [];
    let { timeBasedHistoryRecoders: recoders } =
      await browser.storage.local.get(["timeBasedHistoryRecoders"]);
    recoders = recoders ?? [];
  ...
  async handleRecoders(
    recoders: Array<{ url: string; timestamp: number; tabId: number }>
  ) {
    let tabs = await Promise.all(
      recoders.map(async (recoder) => {
